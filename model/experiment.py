...
entity_model = Word2Vec.load_word2vec_format("prod_embed", "r")

rmse = []
for usr_idx, prod_idx in test_mat.keys():
    true_rating = test_mat[usr2idx, prod_idx]

    #predict
    inds = (train_mat[usr_idx, :] > 0).indices
    inds = [dp.idx2prod[ind] for ind in inds]
    list = entity_model.most_similar(dp.idx2prod[prod_idx], topn=3, restrict_vocab=inds)
